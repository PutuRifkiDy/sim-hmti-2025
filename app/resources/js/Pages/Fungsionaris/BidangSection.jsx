...
/**
 * Props:
 * - buttons: array of string labels (["KOMINFO", "ILMIAH", ..."])
 * - activeIndex: number (index aktif)
 * - onChange: function(index) => void
 * - kabidCard: { nama, jabatan, foto }
 * - kadivCards: [{ nama, jabatan, foto }, ...]
 */
export default function BidangSection({
  ...
  return (
    <div className="flex flex-col items-center text-center pt-[5%] sm:pt-[6%] md:pt-[7%] lg:pt-[8%] w-full max-w-[95%] mx-auto px-4">
      <h1 className="text-[#ecc067] font-poppins font-black leading-[1.1] tracking-[-0.06em] uppercase text-[clamp(2rem,6vw,5.5rem)] -mt-[0.5em]">
        BIDANG
      </h1>
          ...
          {buttons.map((label, idx) => (
            <SwiperSlide key={`bidang-${idx}`} className="flex justify-center !w-auto">
              <Button
                ...
                className={`flex justify-center items-center w-[183px] px-0 py-[8.6px] rounded-[7.215px] font-semibold transition duration-200 mx-auto ${
                  activeIndex === idx
