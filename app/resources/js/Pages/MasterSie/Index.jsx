import { IconMasterSie } from '@/Components/IconAdmin';
...
export default function Index() {
	...
	const total_sie = usePage().props.total_sie;
	const { props } = usePage();
		...
		return (
			<div className="flex flex-col justify-between gap-2 md:flex-row">
				...
				<Button variant="gold" type="button" asChild className="shadow-[0_0_15px_#ECBB4E]">
					<Link as="button" href={route('master-sie.create')} className="py-5 text-[14px] font-bold">
					...
					<div className="mt-5 flex flex-col justify-between gap-2 md:flex-row">
						<div className="flex flex-row gap-10 rounded-[14px] border-2 border-[#ecbb4e]/20 bg-white p-5 dark:bg-[#101010]">
							<div className="flex h-16 w-16 items-center justify-center rounded-2xl bg-[#ecbb4e]/20">
								<IconMasterSie width={30} height={30} />
							</div>
							<div className="flex flex-col">
								<p className="light:text-[#202224]/70 text-[16px] font-medium tracking-[0.03em]">
									Jumlah Sie
								</p>
								<p className="text-[28px] font-bold tracking-[1px]">{total_sie}</p>
							</div>
						</div>
					</div>
										...
										<div className="p-5 dark:bg-[#101010]">
											...
											<div className="mt-10 flex flex-row justify-end gap-5">
												...
												<Button
													...
													onClick={() => {
														router.delete(
															route('master-sie.destroy', { id: selectIdSie }),
															{
																preserveScroll: true,
																preserveState: true,
																onSuccess: () => {
...
Index.layout = (page) => (
	<DashboardLayout
		...
		title={'Master Sie'}
		header={'Master Sie'}
		description={'Kelola master sie di page ini'}
	/>
);
